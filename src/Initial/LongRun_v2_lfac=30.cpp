...
// set Miso for mass regridding - note this is not actually setting Miso for the dynamic calculation
static double Miso_   = 4.e29;
...
// set limit on fraction of total ejecta mass any cell can have
static double massLim = 300.;  

// static variables for integrals; allows calculated values to be used in regridding
static double Eiso  = 0.;                           // initialise for energy calculation (integral)
static double Miso  = 0.;                           // initialise for mass calculation (integral)
static double Viso  = 0.;                           // initialise for ejecta isotropic volume calculation (integral)
static double Vej   = 0.;                           // initialise for ejecta cone volume calculation
static double Mej   = 0.;

// calculate ejecta volume
static double Viso_   = (4./3.)*PI*(rfront*rfront*rfront - rtail_r*rtail_r*rtail_r);
static double theta   = th_simu*(0.5);
static double dTheta  = th_simu;
static double Vej_    = (2./3.)*PI*(rfront*rfront*rfront - rtail_r*rtail_r*rtail_r)*(cos(theta - dTheta/2.) - (cos(theta + dTheta/2.)));

// calculate ejecta mass
static double Mej_    = Miso_ * (Vej_/Viso_);

void loadParams(s_par *par){

  par->tini      = t_ini;             // initial time
  par->ncell[x_] = Nr;              // number of cells in r direction
  par->ncell[y_] = Nth;               // number of cells in theta direction
  par->nmax      = Nr_max;              // max number of cells in MV direction
  ...
  for (int j = 0; j < ncell[F1]; ++j){        // loop through cells along theta
    for (int i = 0; i < ncell[MV]; ++i){      // loop through cells along r
      ...
      if (j == 0){                                          // if in first track
        ...
        double vol  = c->G.dV*lNorm*lNorm*lNorm;
...
int Grid::checkCellForRegrid(int j, int i){
  ...
  double maxMass      = Mej_/massLim;                 // maximum mass allowed in cell
  double cellDensity  = c.S.prim[RHO]*rhoNorm;        // density in cell
  double cellVol      = c.G.dV*lNorm*lNorm*lNorm;     // volume of cell
  double cellMass     = cellDensity*cellVol;          // mass in cell
...
void Cell::user_regridVal(double *res){
  ...
  double maxMass      = Mej_/massLim;                  // maximum mass allowed in cell
  double cellDensity  = S.prim[RHO]*rhoNorm;          // density in cell
  double cellVol      = G.dV*lNorm*lNorm*lNorm;       // volume of cell
  double cellMass     = cellDensity*cellVol;          // mass in cell
  if (cellMass > maxMass){                  // if cell contains more than limit
    *res = -1;                              // make cell exempt from being a regrid victim
  }
  UNUSED(*res);
