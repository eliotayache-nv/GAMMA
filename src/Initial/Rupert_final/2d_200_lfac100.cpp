...
static double lfac    = 100.;                 // --:      energy per unit mass ~ coasting Lorentz factor
static double Eiso    = 1.e52;                // erg:     isotropic equivalent energy
static double R0      = 4.;                  // ls:      initial shell width
...
static double rmin    = 4.e-4;                  // light-seconds, box inner boundary at t_ini
static double rmax    = 6.;                 // light-seconds, box outer boundary at t_ini
  ...
  for (int j = 0; j < ncell[F1]; ++j){        // loop through cells along theta
    for (int i = 0; i < ncell[MV]; ++i){      // loop through cells along r
      ...
      if (r <= R0){
        ...
        c->S.cons[TR1] = 1.;
        c->S.cons2prim(r);
        // cons2prim is allowed since initial velocities are zero.
