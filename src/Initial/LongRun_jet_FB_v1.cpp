...
// simulation settings
static double th_simu = (PI/32.)/100.;        // rad:     simulation angle
...
static double eta     = 1.e-3;                // p0 = rho0*c2, since cold external medium
...
void Grid::userBoundaries(int it, double t){
  ...
  // reflective BC at inner/outer boundaries

  for (int j = 0; j < nde_nax[F1]; ++j){
    for (int i = 0; i < ngst; ++i){
      int nt = ntrack[j];
      Ctot[j][i].S.prim[UU1] *= -1;               // reflecting inner boundary
      // Ctot[j][nt-1-i].S.prim[UU1] *= -1;          // reflecting outer boundary; do not activate this!
    }
  }
...
int Grid::checkCellForRegrid(int j, int i){
  ...
  double r_AR_1 = 1.e4;
  double final_AR = 1.;
  double target_AR  = fmax(pow(r_AR_1/r, 0.2), 1.)*final_AR;
